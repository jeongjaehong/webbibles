...
public class DaoCreator {
	...
	public void onCreateBible(SQLiteDatabase db, int start) {
		...
		for (int i = start; i < limit; i++) {
			...
			try {
				...
				Log.d("onCreate", "Create....Bible Database="
						+ query.toString());
	...
	public void onCreateTempView(SQLiteDatabase db) {

		StringBuffer query = null;
		try {
			query = new StringBuffer();
			query.append("create view v_temp as ");
			query.append("select    a._id songid, 'Download' status from biblenames a ");
			query.append("     union all ");
			query.append("     select   distinct a._id + 66, 'Download' from biblenames a ");
			query.append("     union all ");
			query.append("     select   distinct a._id + 132, 'Download' from biblenames a ");
			query.append("     union all ");
			query.append("     select   distinct a._id + 198, 'Download' from biblenames a ");
			query.append("     union all ");
			query.append("     select   distinct a._id + 265, 'Download' from biblenames a ");
			query.append("     union all ");
			query.append("     select   distinct a._id + 331, 'Download' from biblenames a ");
			query.append("     union all ");
			query.append("     select   distinct a._id + 397, 'Download' from biblenames a ");
			query.append("     union all ");
			query.append("     select   distinct a._id + 463, 'Download' from biblenames a ");
			query.append("     union all ");
			query.append("     select   distinct a._id + 529, 'Download' from biblenames a ");
			query.append("     union all ");
			query.append("     select   distinct a._id + 595, 'Download' from biblenames a ");
			query.append("     order by 1   ");

			db.execSQL("drop view if exists v_temp ");
	...
	public void onCreateBookmark(SQLiteDatabase db, int start) {
		...
		try {
			...
			Log
					.d("onCreate", "Create....Bookmark Database="
							+ query.toString());
	...
	public void onCreateFavorites(SQLiteDatabase db) {
		...
		try {
			...
			// FavoriteGroup.FAVORITEGROUP_TABLE_NAME);
			// db.execSQL("DROP TABLE IF EXISTS " +
			// Favorites.FAVORITES_TABLE_NAME);
			...
			Log.d("onCreate", "Create....Favorites Database="
					+ query.toString());
	...
	public void onCreateSchedule(SQLiteDatabase db) {
		try {
			...
			Log
					.d("onCreate", "Create....Schedule Database="
							+ query.toString());
	...
	public void onCreateIndex(SQLiteDatabase db, int start) {
		...
		for (int i = start; i < limit; i++) {
			try {
				db.execSQL("CREATE INDEX idx_" + Bibles.VERSION_TABLE_NAME[i]
						+ "_01 ON " + Bibles.VERSION_TABLE_NAME[i]
						+ "  (version ASC) ");
				db.execSQL("CREATE INDEX idx_" + Bibles.VERSION_TABLE_NAME[i]
						+ "_02 ON " + Bibles.VERSION_TABLE_NAME[i]
						+ "  (vercode ASC) ");
				db.execSQL("CREATE INDEX idx_" + Bibles.VERSION_TABLE_NAME[i]
						+ "_03 ON " + Bibles.VERSION_TABLE_NAME[i]
						+ "  (vercode, book, chapter ASC) ");
				db.execSQL("CREATE INDEX idx_" + Bibles.VERSION_TABLE_NAME[i]
						+ "_04 ON " + Bibles.VERSION_TABLE_NAME[i]
						+ "  (book, chapter ASC) ");
			} catch (Exception e) {
		...
		for (int i = start; i < limit; i++) {
			...
			try {
				query.append("ALTER TABLE " + Bibles.VERSION_TABLE_NAME[i]
						+ " ");
				query.append(" ADD " + Bibles.STYLE
						+ " INTEGER DEFAULT 0 NOT NULL ");

				Log
						.d("onCreate", "ALTER....Bible Database="
								+ query.toString());
	...
	public void onCreateBibles(Context context, SQLiteDatabase db) {
		...
		String[] bibles = context.getResources().getStringArray(
				R.array.short_biblenames);

		try {
			db.execSQL("drop table if exists biblenames ");

			db
					.execSQL("create table biblenames (_id integer, biblename varchar) ");
			for (int i = 0; i < bibles.length; i++) {
				db.execSQL("INSERT INTO biblenames (_id , biblename) values ("
						+ i + ", '" + bibles[i] + "')");
			}
		} catch (Exception e) {
	...
	public void onUpgrade(Context context, SQLiteDatabase db, int oldVersion,
			int newVersion) {

		Log.d("onUpgrade_Schedule", "oldVersion=" + oldVersion + ",newVersion="
				+ newVersion);
